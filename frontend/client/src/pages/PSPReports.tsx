...
export default function PSPReports() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "issues": return <Badge className="bg-yellow-500/20 text-yellow-400 border-0"><AlertTriangle className="w-3 h-3 mr-1" />Issues</Badge>;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">PSP Reports</h1>
          <p className="text-slate-400 text-sm mt-1">Pre-Employment Screening Program</p>
        </div>
        <Button className="bg-gradient-to-r from-cyan-600 to-emerald-600 hover:from-cyan-700 hover:to-emerald-700 rounded-lg">
          <Plus className="w-4 h-4 mr-2" />Request PSP
        </Button>
      </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-yellow-400">{stats?.issues || 0}</p>}<p className="text-xs text-slate-400">Issues</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
                ...
                <div key={report.id} className={cn("p-4 flex items-center justify-between", report.status === "issues" && "bg-yellow-500/5 border-l-2 border-yellow-500", report.status === "critical" && "bg-red-500/5 border-l-2 border-red-500")}>
                  <div className="flex items-center gap-4">
                    <div className={cn("p-3 rounded-xl", report.status === "clear" ? "bg-green-500/20" : report.status === "issues" ? "bg-yellow-500/20" : "bg-red-500/20")}>
                      <FileText className={cn("w-5 h-5", report.status === "clear" ? "text-green-400" : report.status === "issues" ? "text-yellow-400" : "text-red-400")} />
                    </div>
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-sm text-slate-400">
                        <User className="w-3 h-3" /><span>CDL: {report.cdlNumber}</span>
                      </div>
                      <div className="flex items-center gap-4 text-xs text-slate-500 mt-1">
                        <span>Pulled: {report.pulledDate}</span>
                        <span>Crashes: {report.crashCount}</span>
                        <span>Inspections: {report.inspectionCount}</span>
