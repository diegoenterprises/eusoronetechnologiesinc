...
const PRIVACY_SECTIONS: Section[] = [
  ...
  },
  {
    ...
    content: (
      <div className="space-y-3">
        <p>We implement technical and organizational security measures designed to protect your personal information. The following describes our current security practices:</p>
        <p><strong className="text-white">7.1 Encryption in Transit:</strong> All connections to the Platform are served over HTTPS with TLS encryption. We enforce HTTPS via HTTP Strict Transport Security (HSTS) headers with a one-year max-age, includeSubDomains, and preload directives. HTTP requests in production are automatically redirected (301) to HTTPS. The specific TLS protocol version (e.g., TLS 1.2 or 1.3) is determined by our hosting infrastructure and kept current with industry standards.</p>
        <p><strong className="text-white">7.2 Encryption at Rest:</strong> Sensitive financial data, including bank account numbers, is encrypted using AES-256-CBC with random initialization vectors before storage. Passwords are hashed using bcryptjs with a cost factor of 12 (one-way hashing; passwords cannot be decrypted). Database-level encryption at rest is provided by our cloud database provider's native encryption capabilities.</p>
        <p><strong className="text-white">7.3 Authentication & Access Control:</strong> User sessions are managed via JSON Web Tokens (JWT) with 7-day expiration. The Platform implements role-based access control (RBAC) with three access tiers: public endpoints (no authentication required), protected endpoints (authenticated users only), and admin endpoints (restricted to ADMIN and SUPER_ADMIN roles). This ensures users can only access data and features appropriate to their assigned role.</p>
        <p><strong className="text-white">7.4 API Protection:</strong> The Platform employs rate limiting to prevent abuse: general API endpoints are limited to 100 requests per minute, authentication endpoints are limited to 10 attempts per 15 minutes, and file upload endpoints are limited to 50 requests per hour. Rate limit headers (X-RateLimit-Limit, X-RateLimit-Remaining, X-RateLimit-Reset) are included in all API responses.</p>
        <p><strong className="text-white">7.5 Security Headers:</strong> All responses include security headers: Content-Security-Policy (CSP) restricting script, style, and resource sources; X-XSS-Protection; X-Content-Type-Options (nosniff); X-Frame-Options (SAMEORIGIN) to prevent clickjacking; strict Referrer-Policy; and Permissions-Policy restricting access to device APIs. Server identification headers (X-Powered-By) are removed.</p>
        <p><strong className="text-white">7.6 Input Validation & Sanitization:</strong> All API inputs are validated using schema-based validation (Zod). Query parameters are sanitized to remove potential XSS vectors. Content Security Policy directives restrict script execution to approved sources.</p>
        <p><strong className="text-white">7.7 Payment Processing:</strong> Credit and debit card transactions are processed through Stripe, a PCI-DSS Level 1 certified payment processor. We do not store, process, or transmit raw credit card numbers on our servers. All card data is handled entirely by Stripe's PCI-compliant infrastructure. Bank account data processed through our platform is encrypted as described in Section 7.2.</p>
        <p><strong className="text-white">7.8 CORS & Cross-Origin Protection:</strong> Cross-Origin Resource Sharing (CORS) is configured to restrict API access to authorized origins in production. Credentials are required for cross-origin requests. Pre-flight request caching is set to 24 hours.</p>
        <p><strong className="text-white">7.9 Breach Notification:</strong> In the event of a data breach affecting your personal information, we will notify affected Users and applicable regulatory authorities as required by applicable state breach notification laws (e.g., Texas Business and Commerce Code Chapter 521, California Civil Code 1798.82, etc.) within the timeframes required by law.</p>
        <p>Despite our security measures, no method of electronic transmission or storage is 100% secure. We cannot guarantee absolute security. We continuously work to improve our security practices and will implement additional measures such as third-party security audits and certifications as the Platform scales.</p>
      </div>
    ),
  },
  {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <Card className="bg-emerald-500/10 border-emerald-500/30 rounded-xl mb-6">
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              <Lock className="w-5 h-5 text-emerald-400 mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-emerald-300 font-semibold text-sm">YOUR PRIVACY MATTERS</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl mt-8">
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-white font-semibold">Privacy Questions or Data Requests?</p>
