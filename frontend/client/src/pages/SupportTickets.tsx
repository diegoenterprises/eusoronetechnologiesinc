...
export default function SupportTickets() {
  ...
  const ticketsQuery = trpc.support.getTickets.useQuery({ search, status });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      {showNew && (
        <Card className="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 border-cyan-500/30 rounded-xl">
          ...
          <CardContent className="space-y-4">
            <div><label className="text-sm text-slate-400 mb-1 block">Subject</label><Input value={newTicket.subject} onChange={(e) => setNewTicket(prev => ({ ...prev, subject: e.target.value }))} placeholder="Brief description of your issue" className="bg-slate-700/50 border-slate-600/50 rounded-lg" /></div>
            ...
            <div><label className="text-sm text-slate-400 mb-1 block">Message</label><Textarea value={newTicket.message} onChange={(e) => setNewTicket(prev => ({ ...prev, message: e.target.value }))} placeholder="Describe your issue in detail" className="bg-slate-700/50 border-slate-600/50 rounded-lg min-h-[100px]" /></div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tickets..." className="pl-9 bg-slate-800/50 border-slate-700/50 rounded-lg" /></div>
