    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      {showAD && (
        ...
        <div className="fixed inset-0 z-[9999] bg-black/50 overflow-y-auto" onClick={() => setShowAD(false)}>
          ...
          <div role="dialog" aria-modal="true" aria-label="Add driver" className={cn("w-full max-w-lg rounded-2xl p-6 space-y-4 shadow-2xl backdrop-blur-2xl", L ? "bg-white/90 border border-slate-200" : "bg-slate-900/70 border border-white/10 ring-1 ring-white/5")} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between"><h2 className="text-lg font-bold bg-gradient-to-r from-[#1473FF] to-[#BE01FF] bg-clip-text text-transparent">Add Driver</h2><button aria-label="Close add driver dialog" onClick={() => setShowAD(false)}><X className="w-5 h-5 text-slate-400" aria-hidden="true" /></button></div>
      ...
      {showAV && (
        ...
        <div className="fixed inset-0 z-[9999] bg-black/50 overflow-y-auto" onClick={() => setShowAV(false)}>
          <div className="flex min-h-full items-center justify-center p-4">
          <div role="dialog" aria-modal="true" aria-label="Add vehicle" className={cn("w-full max-w-lg rounded-2xl p-6 space-y-4 shadow-2xl backdrop-blur-2xl", L ? "bg-white/90 border border-slate-200" : "bg-slate-900/70 border border-white/10 ring-1 ring-white/5")} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between"><h2 className="text-lg font-bold bg-gradient-to-r from-[#1473FF] to-[#BE01FF] bg-clip-text text-transparent">Add Vehicle</h2><button aria-label="Close add vehicle dialog" onClick={() => setShowAV(false)}><X className="w-5 h-5 text-slate-400" aria-hidden="true" /></button></div>
