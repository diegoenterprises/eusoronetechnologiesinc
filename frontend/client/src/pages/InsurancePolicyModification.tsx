...
export default function InsurancePolicyModification() {
  ...
  const dataQuery = (trpc as any).insurance.getPolicyModification.useQuery();
  const data = dataQuery.data;

  // tRPC Mutations
  const createMutation = (trpc as any).insurance.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = (trpc as any).insurance.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = (trpc as any).insurance.delete.useMutation({
    onSuccess: () => {
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Modify policies</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search policy modification..."
          value={searchQuery}
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-lg">Policy Modification</CardTitle>
            <div className="flex gap-2">
              <Button 
                size="sm" 
                variant="ghost" 
              ...
              {items
                ...
                .map((item: any, idx: number) => (
                  <div
                    ...
                    onClick={() => setLocation(`/insurance/${item.id}`)}
                  >
                    <div className="flex items-center gap-4">
