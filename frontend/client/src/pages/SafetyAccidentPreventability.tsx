...
export default function SafetyAccidentPreventability() {
  ...
  const dataQuery = (trpc as any).safety.getPreventabilityReview.useQuery();
  const data = dataQuery.data;

  // tRPC Mutations
  const createMutation = (trpc as any).safety.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = (trpc as any).safety.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = (trpc as any).safety.delete.useMutation({
    onSuccess: () => {
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Preventability determination</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button 
            className="bg-gradient-to-r from-red-600 to-rose-600 rounded-lg"
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search preventability review..."
          value={searchQuery}
          onChange={(e: any) => setSearchQuery(e.target.value)}
          className="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-red-500/50"
        />
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-lg">Preventability Review</CardTitle>
            <div className="flex gap-2">
              <Button 
                size="sm" 
                variant="ghost" 
        ...
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Button 
                className="mt-4 bg-gradient-to-r from-red-600 to-rose-600 rounded-lg"
              ...
              {items
                ...
                .map((item: any, idx: number) => (
                  <div
                    ...
                    onClick={() => setLocation(`/safety/${item.id}`)}
                  >
                    <div className="flex items-center gap-4">
