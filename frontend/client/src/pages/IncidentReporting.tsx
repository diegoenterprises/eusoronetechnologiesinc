...
export default function IncidentReporting() {
  ...
  const incidentsQuery = (trpc as any).safety.getIncidents.useQuery({ filter });
  ...
  const getSeverityBadge = (severity: string) => {
    switch (severity) {
      ...
      case "major": return <Badge className="bg-orange-500/20 text-orange-400 border-0">Major</Badge>;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Incident Reporting</h1>
          <p className="text-slate-400 text-sm mt-1">Report and track incidents</p>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-yellow-400">{stats?.investigating || 0}</p>}<p className="text-xs text-slate-400">Investigating</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3"><CardTitle className="text-white text-lg flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-cyan-400" />Incidents</CardTitle></CardHeader>
        <CardContent className="p-0">
          {incidentsQuery.isLoading ? (
            ...
            <div className="text-center py-16"><CheckCircle className="w-10 h-10 text-green-500 mx-auto mb-3" /><p className="text-slate-400">No incidents</p></div>
          ) : (
            <div className="divide-y divide-slate-700/50">
              {(incidentsQuery.data as any)?.map((incident: any) => (
                <div key={incident.id} className={cn("p-4 flex items-center justify-between", incident.severity === "critical" && "bg-red-500/5 border-l-2 border-red-500")}>
                  <div className="flex items-center gap-4">
                    <div className={cn("p-3 rounded-xl", incident.severity === "critical" ? "bg-red-500/20" : incident.severity === "major" ? "bg-orange-500/20" : "bg-yellow-500/20")}>
                      <AlertTriangle className={cn("w-5 h-5", incident.severity === "critical" ? "text-red-400" : incident.severity === "major" ? "text-orange-400" : "text-yellow-400")} />
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-white font-bold">#{incident.id}</p>
                        {getSeverityBadge(incident.severity)}
                        {getStatusBadge(incident.status)}
                      </div>
                      <p className="text-sm text-slate-400">{incident.type}: {incident.description}</p>
