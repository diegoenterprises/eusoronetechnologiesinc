...
export default function AuditLogs() {
  ...
  const logsQuery = trpc.admin.getAuditLogs.useQuery({ filter, search });
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      ...
      case "permission": return <Badge className="bg-purple-500/20 text-purple-400 border-0">Permission</Badge>;
    ...
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">System activity and security logs</p>
        </div>
      </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-cyan-400">{stats?.totalToday || 0}</p>}<p className="text-xs text-slate-400">Today</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-green-400">{stats?.logins || 0}</p>}<p className="text-xs text-slate-400">Logins</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-full bg-blue-500/20"><Settings className="w-6 h-6 text-blue-400" /></div>
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-blue-400">{stats?.changes || 0}</p>}<p className="text-xs text-slate-400">Changes</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-red-400">{stats?.highSeverity || 0}</p>}<p className="text-xs text-slate-400">High Severity</p></div>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3"><CardTitle className="text-white text-lg flex items-center gap-2"><FileText className="w-5 h-5 text-cyan-400" />Activity Log</CardTitle></CardHeader>
          ...
          {logsQuery.isLoading ? (
            ...
            <div className="divide-y divide-slate-700/50">
              {logsQuery.data?.map((log: any) => (
                <div key={log.id} className={cn("p-4", log.severity === "high" && "bg-red-500/5 border-l-2 border-red-500")}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="p-2 rounded-lg bg-slate-700/50">{getSeverityIcon(log.severity)}</div>
                      <div>
