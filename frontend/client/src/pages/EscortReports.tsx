...
import {
  FileText, Search, CheckCircle, Clock, Download,
  Plus, Eye
} from "lucide-react";
import { cn } from "@/lib/utils";

export default function EscortReports() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "draft": return <Badge className="bg-yellow-500/20 text-yellow-400 border-0"><Clock className="w-3 h-3 mr-1" />Draft</Badge>;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Escort Reports</h1>
          <p className="text-slate-400 text-sm mt-1">View and manage your escort reports</p>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-green-400">{stats?.submitted || 0}</p>}<p className="text-xs text-slate-400">Submitted</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-yellow-400">{stats?.drafts || 0}</p>}<p className="text-xs text-slate-400">Drafts</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        ...
        <CardContent className="p-0">
          {reportsQuery.isLoading ? (
            ...
            <div className="divide-y divide-slate-700/50">
              {(Array.isArray(reportsQuery.data) ? reportsQuery.data : []).map((report: any) => (
                <div key={report.id} className="p-4 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={cn("p-3 rounded-xl", report.status === "submitted" ? "bg-green-500/20" : report.status === "draft" ? "bg-yellow-500/20" : "bg-blue-500/20")}>
                      <FileText className={cn("w-5 h-5", report.status === "submitted" ? "text-green-400" : report.status === "draft" ? "text-yellow-400" : "text-blue-400")} />
                    </div>
                    <div>
