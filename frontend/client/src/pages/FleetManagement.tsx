...
import {
  Truck, Users, Wrench, AlertTriangle, Search, Plus,
  Eye, MapPin, CheckCircle
} from "lucide-react";
...
export default function FleetManagement() {
  ...
  const vehiclesQuery = trpc.fleet.list.useQuery({ limit: 50 });
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage your vehicles and drivers</p>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                {summaryQuery.isLoading ? <Skeleton className="h-8 w-12" /> : (
                  <p className="text-2xl font-bold text-green-400">{summary?.activeVehicles || 0}</p>
                )}
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                {summaryQuery.isLoading ? <Skeleton className="h-8 w-12" /> : (
                  <p className="text-2xl font-bold text-yellow-400">{summary?.inMaintenance || 0}</p>
                )}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-slate-800/50 border border-slate-700/50 rounded-lg p-1">
          <TabsTrigger value="vehicles" className="data-[state=active]:bg-slate-700 rounded-md">Vehicles</TabsTrigger>
            ...
            <CardContent className="p-0">
              {vehiclesQuery.isLoading ? (
                ...
                <div className="divide-y divide-slate-700/50">
                  {vehiclesQuery.data?.vehicles?.map((vehicle: any) => (
