...
export default function TrainingLMS() {
  ...
  // Auto-open course from ?course= query param (supports numeric ID or slug)
  const urlParams = typeof window !== "undefined" ? new URLSearchParams(window.location.search) : null;
  const courseFromUrl = urlParams?.get("course") || null;
  const courseIsNumeric = courseFromUrl ? /^\d+$/.test(courseFromUrl) : false;

  const [view, setView] = useState<ViewMode>(courseFromUrl ? "detail" : "catalog");
  const [selectedCourseId, setSelectedCourseId] = useState<number | null>(courseIsNumeric ? parseInt(courseFromUrl!, 10) : null);
  const [selectedCourseSlug, setSelectedCourseSlug] = useState<string>(courseIsNumeric ? "" : (courseFromUrl || ""));
