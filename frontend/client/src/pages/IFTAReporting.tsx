  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">IFTA Reporting</h1>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-16" /> : <p className="text-2xl font-bold text-green-400">{stats?.totalGallons?.toLocaleString()}</p>}<p className="text-xs text-slate-400">Gallons</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
      ...
      {report?.status && (
        <Card className={cn("rounded-xl", report.status === "filed" ? "bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-500/30" : report.status === "pending" ? "bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-yellow-500/30" : "bg-slate-800/50 border-slate-700/50")}>
