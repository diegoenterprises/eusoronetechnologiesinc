              ...
              {(documentsQuery.data as any)?.map((doc: any) => (
                <div key={doc.id} className={cn("p-4 flex items-center justify-between", doc.status === "expired" && "bg-red-500/5 border-l-2 border-red-500")}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button size="sm" variant="outline" className="bg-red-500/20 border-red-500/30 text-red-400 rounded-lg" onClick={() => setDeleteId(doc.id)}><Trash2 className="w-4 h-4" /></Button>
                  </div>
                </div>
              ))}
            </div>
