...
export default function CatalystDetails() {
  ...
  const catalystId = params.id as string;

  const catalystQuery = (trpc as any).catalysts.getById.useQuery({ id: catalystId });
  const driversQuery = (trpc as any).catalysts.getDrivers.useQuery({ catalystId, limit: 10 });
  const loadsQuery = (trpc as any).catalysts.getRecentLoads.useQuery({ catalystId, limit: 5 });
    ...
    return (
      <div className="p-4 md:p-6">
        <div className="text-center py-16">
          ...
          <p className="text-slate-400 text-lg">Catalyst not found</p>
          <Button className="mt-4 bg-slate-700 hover:bg-slate-600 rounded-lg" onClick={() => setLocation("/catalyst/dashboard")}>
            <ArrowLeft className="w-4 h-4 mr-2" />Back to Catalysts
          </Button>
        </div>
      </div>
    );
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" className="text-slate-400 hover:text-white" onClick={() => setLocation("/catalyst/dashboard")}>
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">MC# {catalyst.mcNumber}</p>
          </div>
        </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-yellow-400">{catalyst.rating?.toFixed(1) || "N/A"}</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-blue-400">{catalyst.loadsCompleted || 0}</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold bg-gradient-to-r from-[#1473FF] to-[#BE01FF] bg-clip-text text-transparent">{catalyst.onTimeRate || 0}%</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-purple-400">{catalyst.driverCount || 0}</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-bold text-cyan-400">{catalyst.fleetSize || 0}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          ...
          <CardContent className="space-y-3">
            <div className="p-3 rounded-xl bg-slate-700/30">
              ...
              <p className="text-white font-medium">{catalyst.dotNumber}</p>
            </div>
            <div className="p-3 rounded-xl bg-slate-700/30">
              <p className="text-xs text-slate-500">MC Number</p>
              <p className="text-white font-medium">{catalyst.mcNumber}</p>
            </div>
            <div className="p-3 rounded-xl bg-slate-700/30">
            ...
            <div className="p-3 rounded-xl bg-slate-700/30">
              ...
              <p className="text-white font-medium">{typeof catalyst.address === "object" ? `${catalyst.address.street}, ${catalyst.address.city}, ${catalyst.address.state} ${catalyst.address.zip}` : catalyst.address}</p>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 rounded-xl bg-slate-700/30">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={cn("font-bold", catalyst.safetyRating === "Satisfactory" ? "text-green-400" : "text-yellow-400")}>
                  {catalyst.safetyRating || "N/A"}
                </span>
              </div>
            </div>
            <div className="p-4 rounded-xl bg-slate-700/30">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge className={catalyst.insuranceValid ? "bg-green-500/20 text-green-400 border-0" : "bg-red-500/20 text-red-400 border-0"}>
                  {catalyst.insuranceValid ? "Valid" : "Expired"}
                </Badge>
              </div>
              <p className="text-xs text-slate-500">Expires: {catalyst.insuranceExpiry}</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-700/30">
              <div className="flex items-center justify-between mb-2">
                <span className="text-slate-400">Authority</span>
                <Badge className={catalyst.authorityActive ? "bg-green-500/20 text-green-400 border-0" : "bg-red-500/20 text-red-400 border-0"}>
                  {catalyst.authorityActive ? "Active" : "Inactive"}
                </Badge>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 rounded-xl bg-slate-700/30 text-center">
                <p className="text-lg font-bold text-blue-400">{catalyst.csaScore || "N/A"}</p>
                <p className="text-xs text-slate-500">CSA Score</p>
              </div>
              <div className="p-3 rounded-xl bg-slate-700/30 text-center">
                <p className="text-lg font-bold text-purple-400">{catalyst.inspections || 0}</p>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          ...
          <CardContent>
            {catalyst.equipmentTypes?.length === 0 ? (
              <p className="text-slate-400 text-center py-4">No equipment listed</p>
            ) : (
              <div className="flex flex-wrap gap-2">
                {catalyst.equipmentTypes?.map((type: string, idx: number) => (
            ...
            <div className="mt-6 space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {catalyst.serviceAreas?.map((area: string, idx: number) => (
