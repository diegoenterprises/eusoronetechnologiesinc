...
export default function CatalystVetting() {
  const [search, setSearch] = useState("");

  const catalystsQuery = (trpc as any).brokers.getPendingVetting.useQuery({ search });
  const statsQuery = (trpc as any).brokers.getVettingStats.useQuery();

  const approveMutation = (trpc as any).brokers.approveCatalyst.useMutation({
    onSuccess: () => { toast.success("Catalyst approved"); catalystsQuery.refetch(); statsQuery.refetch(); },
    onError: (error: any) => toast.error("Failed", { description: error.message }),
  });

  const rejectMutation = (trpc as any).brokers.rejectCatalyst.useMutation({
    onSuccess: () => { toast.success("Catalyst rejected"); catalystsQuery.refetch(); statsQuery.refetch(); },
    onError: (error: any) => toast.error("Failed", { description: error.message }),
  });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-[#1473FF] to-[#BE01FF] bg-clip-text text-transparent">Catalyst Vetting</h1>
          <p className="text-slate-400 text-sm mt-1">Verify and approve catalysts</p>
        </div>
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        ...
        <CardContent className="p-0">
          {catalystsQuery.isLoading ? (
            ...
            <div className="text-center py-16"><CheckCircle className="w-10 h-10 text-green-500 mx-auto mb-3" /><p className="text-slate-400">No pending catalysts</p></div>
          ) : (
            <div className="divide-y divide-slate-700/50">
              {(catalystsQuery.data as any)?.map((catalyst: any) => (
                <div key={catalyst.id} className="p-4">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-white font-bold text-lg">{catalyst.name}</p>
                    ...
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="outline" className="bg-red-500/20 border-red-500/30 text-red-400 hover:bg-red-500/30 rounded-lg" onClick={() => rejectMutation.mutate({ catalystId: catalyst.id })}>
                        <XCircle className="w-4 h-4 mr-1" />Reject
                      </Button>
                      <Button size="sm" className="bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 rounded-lg" onClick={() => approveMutation.mutate({ catalystId: catalyst.id })}>
                  ...
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-sm text-white">{catalyst.checksCompleted}/{catalyst.totalChecks} checks</span>
                    </div>
                    <Progress value={(catalyst.checksCompleted / catalyst.totalChecks) * 100} className="h-2" />
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {catalyst.checks?.map((check: any) => (
