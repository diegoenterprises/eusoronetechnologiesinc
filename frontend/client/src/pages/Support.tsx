...
function AdminSupportView() {
  ...
  const adminKBArticles = [
    ...
    { title: "Invoicing & Payments", desc: "Invoicing and settlement", icon: <CreditCard className="w-5 h-5 text-blue-500" />, category: "Billing" },
...
function UserSupportView() {
  ...
  const kbArticles: { title: string; desc: string; icon: React.ReactNode; category: string; roles: string[]; content: string }[] = [
    ...
    { title: "Invoicing & Payments", desc: "Invoicing, payment processing, and settlement", icon: <CreditCard className="w-5 h-5 text-blue-500" />, category: "Billing", roles: ["SHIPPER", "CATALYST", "BROKER"],
      content: "EusoTrip handles invoicing and payment processing seamlessly. After delivery, invoices are generated with load details, rate, and platform fee breakdown. Shippers can pay via credit card, ACH, or wire transfer. Catalysts receive settlements to their connected EusoConnect account. View all invoices, payment history, and outstanding balances in the Payments section. Email notifications are sent for every payment event." },
