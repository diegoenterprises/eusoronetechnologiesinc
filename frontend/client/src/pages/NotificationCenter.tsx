...
import {
  ...
  CheckCheck, Package, DollarSign, Shield
} from "lucide-react";
...
export default function NotificationCenter() {
  ...
  const getTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "load": return <Package className="w-5 h-5 text-cyan-400" />;
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-green-400">{stats?.read || 0}</p>}<p className="text-xs text-slate-400">Read</p></div>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {notificationsQuery.isLoading ? (
            ...
            <div className="divide-y divide-slate-700/50">
              {notificationsQuery.data?.map((notification: any) => (
                <div key={notification.id} className={cn("p-4 flex items-start gap-4", !notification.read && "bg-cyan-500/5 border-l-2 border-cyan-500")} onClick={() => !notification.read && markReadMutation.mutate({ id: notification.id })}>
                  <div className={cn("p-2 rounded-full", notification.type === "error" ? "bg-red-500/20" : notification.type === "warning" ? "bg-yellow-500/20" : notification.type === "success" ? "bg-green-500/20" : "bg-slate-700/50")}>
