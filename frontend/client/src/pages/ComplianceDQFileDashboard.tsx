/**
 * COMPLIANCEDQFILEDASHBOARD
 * Driver qualification overview
...
export default function ComplianceDQFileDashboard() {
  ...
  const dataQuery = (trpc as any).compliance.getDqFileDashboard.useQuery();
  const data = dataQuery.data;

  // tRPC Mutations
  const createMutation = (trpc as any).compliance.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = (trpc as any).compliance.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = (trpc as any).compliance.delete.useMutation({
    onSuccess: () => {
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Driver qualification overview</p>
        </div>
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search dq file dashboard..."
          value={searchQuery}
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-lg">DQ File Dashboard</CardTitle>
            <div className="flex gap-2">
              <Button 
                size="sm" 
                variant="ghost" 
        ...
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Button 
                className="mt-4 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-lg"
              ...
              {items
                ...
                .map((item: any, idx: number) => (
                  <div
                    ...
                    onClick={() => setLocation(`/compliance/${item.id}`)}
                  >
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={(e: any) => {
                          e.stopPropagation();
                          setLocation(`/compliance/${item.id}`);
                        }}
                      >
