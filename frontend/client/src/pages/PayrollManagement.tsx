...
import {
  DollarSign, Users, CheckCircle, Clock, Calendar,
  Download
} from "lucide-react";
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Payroll Management</h1>
          <p className="text-slate-400 text-sm mt-1">Driver payroll processing</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" className="bg-slate-800/50 border-slate-700/50 rounded-lg"><Download className="w-4 h-4 mr-2" />Export</Button>
          <Button className="bg-gradient-to-r from-cyan-600 to-emerald-600 hover:from-cyan-700 hover:to-emerald-700 rounded-lg" onClick={() => processMutation.mutate({ period })}>
            <DollarSign className="w-4 h-4 mr-2" />Process Payroll
          </Button>
        </div>
      </div>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-12" /> : <p className="text-2xl font-bold text-cyan-400">{stats?.employees || 0}</p>}<p className="text-xs text-slate-400">Employees</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>{statsQuery.isLoading ? <Skeleton className="h-8 w-20" /> : <p className="text-2xl font-bold text-purple-400">${stats?.totalNet?.toLocaleString()}</p>}<p className="text-xs text-slate-400">Net Pay</p></div>
            </div>
          </CardContent>
        </Card>
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
              ...
              {(payrollQuery.data as any)?.map((record: any) => (
                <div key={record.id} className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-right">
                      <p className="text-lg font-bold text-purple-400">${record.netPay?.toLocaleString()}</p>
