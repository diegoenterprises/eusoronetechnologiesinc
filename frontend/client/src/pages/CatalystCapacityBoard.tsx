...
export default function CatalystCapacityBoard() {
  const [search, setSearch] = useState("");
  const [equipment, setEquipment] = useState("all");

  const catalystsQuery = (trpc as any).brokers.getCatalystCapacity.useQuery({ search, equipment });
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-[#1473FF] to-[#BE01FF] bg-clip-text text-transparent">Catalyst Capacity Board</h1>
          <p className="text-slate-400 text-sm mt-1">Available catalyst capacity</p>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input value={search} onChange={(e: any) => setSearch(e.target.value)} placeholder="Search catalysts or locations..." className="pl-9 bg-slate-800/50 border-slate-700/50 rounded-lg" />
        </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3"><CardTitle className="text-white text-lg flex items-center gap-2"><Truck className="w-5 h-5 text-cyan-400" />Available Catalysts</CardTitle></CardHeader>
              ...
              {(catalystsQuery.data as any)?.map((catalyst: any) => (
                <div key={catalyst.id} className="p-4 hover:bg-slate-700/20 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <div className="flex items-center gap-1 text-yellow-400"><Star className="w-4 h-4 fill-current" /><span className="text-sm">{catalyst.rating}</span></div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div className="flex items-center gap-2 text-slate-400"><MapPin className="w-4 h-4" /><span>{catalyst.location}</span></div>
                    <div className="flex items-center gap-2 text-slate-400"><Truck className="w-4 h-4" /><span>{catalyst.equipment}</span></div>
                    <div className="flex items-center gap-2 text-slate-400"><Package className="w-4 h-4" /><span>{catalyst.availableTrucks} trucks available</span></div>
                    <div className="flex items-center gap-2 text-slate-400"><Clock className="w-4 h-4" /><span>Available: {catalyst.availableDate}</span></div>
                  </div>
                  {catalyst.lanes && catalyst.lanes.length > 0 && (
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      <span className="text-xs text-slate-500">Preferred lanes:</span>
                      {catalyst.lanes.map((lane: string, i: number) => (
