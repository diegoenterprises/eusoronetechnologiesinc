...
export default function PermitManagement() {
  ...
  const permitsQuery = trpc.compliance.getPermits.useQuery({ search });
  const statsQuery = trpc.compliance.getPermitStats.useQuery();
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Permit Management</h1>
      ...
      <div className="relative max-w-sm">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search permits..." className="pl-9 bg-slate-800/50 border-slate-700/50 rounded-lg" />
      </div>
