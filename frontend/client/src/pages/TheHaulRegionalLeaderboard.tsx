...
export default function TheHaulRegionalLeaderboard() {
  ...
  const dataQuery = (trpc as any).gamification.getRegionalLeaderboard.useQuery();
  const data = dataQuery.data;

  // tRPC Mutations
  const createMutation = (trpc as any).gamification.create.useMutation({
    onSuccess: () => {
  ...
  const updateMutation = (trpc as any).gamification.update.useMutation({
    onSuccess: () => {
  ...
  const deleteMutation = (trpc as any).gamification.delete.useMutation({
    onSuccess: () => {
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Regional</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button 
            className="bg-gradient-to-r from-amber-600 to-orange-600 rounded-lg"
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search regional leaderboard..."
          value={searchQuery}
          onChange={(e: any) => setSearchQuery(e.target.value)}
          className="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-amber-500/50"
        />
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-white text-lg">Regional Leaderboard</CardTitle>
            <div className="flex gap-2">
              <Button 
                size="sm" 
                variant="ghost" 
        ...
        <CardContent>
          {items.length === 0 ? (
            <div className="text-center py-12">
              ...
              <Button 
                className="mt-4 bg-gradient-to-r from-amber-600 to-orange-600 rounded-lg"
              ...
              {items
                ...
                .map((item: any, idx: number) => (
                  <div
                    ...
                    onClick={() => setLocation(`/gamification/${item.id}`)}
                  >
                    <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button
                        ...
                        onClick={(e: any) => {
                          e.stopPropagation();
                          setLocation(`/gamification/${item.id}`);
                        }}
                      >
