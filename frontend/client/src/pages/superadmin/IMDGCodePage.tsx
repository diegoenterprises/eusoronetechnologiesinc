  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">International Maritime Dangerous Goods — Multi-modal road + sea hazmat compliance</p>
      </div>
      ...
      <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-slate-500 uppercase">Proper Shipping Name</label>
            ...
            <div>
              <label className="text-xs text-slate-500 uppercase">Packing Group</label>
        ...
        <Card className="bg-blue-500/5 border-blue-500/20 rounded-xl">
          <CardContent className="p-4 space-y-3">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge className="text-xs bg-blue-500/10 border-blue-500/20 text-blue-400">{compliance.imdgClass}</Badge>
              <span className="text-xs text-white font-semibold">{compliance.imdgProperShippingName}</span>
              {compliance.packingGroupCode && <Badge className="text-xs bg-amber-500/10 border-amber-500/20 text-amber-400">PG {compliance.packingGroupCode}</Badge>}
            </div>
            {compliance.packingGroupDescription && <p className="text-xs text-slate-400">{compliance.packingGroupDescription}</p>}
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5">
                <div className={cn("w-2 h-2 rounded-full", compliance.containerPackingCertUrl ? "bg-emerald-400" : "bg-slate-600")} />
                <span className="text-xs text-slate-400">Packing Cert</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={cn("w-2 h-2 rounded-full", compliance.dgDeclarationFormUrl ? "bg-emerald-400" : "bg-slate-600")} />
                <span className="text-xs text-slate-400">DG Declaration</span>
              </div>
              <div className="flex items-center gap-1.5">
                <div className={cn("w-2 h-2 rounded-full", compliance.vesselManifestSubmitted ? "bg-emerald-400" : "bg-slate-600")} />
                <span className="text-xs text-slate-400">Vessel Manifest</span>
              </div>
              {!compliance.vesselManifestSubmitted && (
                <Button variant="ghost" size="sm" className="h-5 px-2 text-xs text-blue-400 hover:bg-blue-500/10" onClick={() => manifestM.mutate({ loadId: lookupLoadId! })}>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardHeader className="pb-2"><CardTitle className="text-xs text-slate-500 uppercase tracking-wider">DOT → IMDG Class Map</CardTitle></CardHeader>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardHeader className="pb-2"><CardTitle className="text-xs text-slate-500 uppercase tracking-wider">Packing Groups (IMDG)</CardTitle></CardHeader>
          <CardContent>
            <div className="space-y-2">
              {packingGroups.map((pg: any) => (
                <div key={pg.code} className="flex items-start gap-2 text-xs">
                  <Badge className="text-xs bg-amber-500/10 border-amber-500/20 text-amber-400 shrink-0">PG {pg.code}</Badge>
