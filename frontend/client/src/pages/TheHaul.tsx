            ...
            <CardContent>
              ...
              : <div className="space-y-2">{(leaderboardQ.data?.leaders || []).map((l: any) => (
                <div key={l.userId} className={cn("flex items-center gap-3 p-3 rounded-xl border", isLight ? "bg-slate-50 border-slate-200" : "bg-slate-800/30 border-slate-700/50")}>
                  <div className={cn("w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm", l.rank === 1 ? "bg-yellow-500/20 text-yellow-400" : l.rank === 2 ? "bg-slate-300/20 text-slate-300" : l.rank === 3 ? "bg-orange-500/20 text-orange-400" : "bg-slate-500/10 text-slate-400")}>{l.rank <= 3 ? ["🥇","🥈","🥉"][l.rank-1] : `#${l.rank}`}</div>
                  <div className="flex-1 min-w-0">
