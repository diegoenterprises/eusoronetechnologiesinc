...
export default function FactoringDashboard() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      {overviewQuery.isLoading ? <Skeleton className="h-32 w-full rounded-xl" /> : account && (
        <Card className="bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-500/30 rounded-xl">
          <CardContent className="p-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div><p className="text-white font-bold text-xl">Factoring Provider</p><p className="text-sm text-slate-400">Account: {account?.status}</p></div>
            </div>
            <div className="grid grid-cols-4 gap-4">
              <div className="p-3 rounded-lg bg-slate-800/50"><p className="text-xs text-slate-500">Credit Limit</p><p className="text-white font-bold">${account?.creditLimit?.toLocaleString()}</p></div>
              <div className="p-3 rounded-lg bg-slate-800/50"><p className="text-xs text-slate-500">Available</p><p className="text-green-400 font-bold">${account?.availableCredit?.toLocaleString()}</p></div>
              <div className="p-3 rounded-lg bg-slate-800/50"><p className="text-xs text-slate-500">Advance Rate</p><p className="text-cyan-400 font-bold">{account?.advanceRate}%</p></div>
              <div className="p-3 rounded-lg bg-slate-800/50"><p className="text-xs text-slate-500">Fee Rate</p><p className="text-purple-400 font-bold">{account?.factoringRate}%</p></div>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid grid-cols-4 gap-4">
        {overviewQuery.isLoading ? Array(4).fill(0).map((_, i) => <Skeleton key={i} className="h-24 rounded-xl" />) : (
          <>
            ...
            <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl"><CardContent className="p-5 flex items-center gap-4"><div className="p-3 rounded-full bg-yellow-500/20"><Receipt className="w-6 h-6 text-yellow-400" /></div><div><p className="text-2xl font-bold text-yellow-400">{stats?.pendingPayments || 0}</p><p className="text-xs text-slate-400">Pending</p></div></CardContent></Card>
            <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl"><CardContent className="p-5 flex items-center gap-4"><div className="p-3 rounded-full bg-purple-500/20"><DollarSign className="w-6 h-6 text-purple-400" /></div><div><p className="text-2xl font-bold text-purple-400">{stats?.feesCharged || 0}</p><p className="text-xs text-slate-400">Fees</p></div></CardContent></Card>
          </>
        )}
      </div>
    </div>
