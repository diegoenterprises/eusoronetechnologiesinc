                ...
                <button key={neg.id} onClick={() => setSelectedId(neg.id)} className="w-full p-4 flex items-center justify-between hover:bg-slate-700/20 transition-colors text-left">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <Badge className={cn("text-xs shrink-0", STATUS_COLORS[neg.status])}>{neg.status?.replace(/_/g, " ")}</Badge>
                    </div>
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                      <span>#{neg.negotiationNumber}</span>
                      <Badge variant="outline" className="text-xs border-slate-600">{TYPE_LABELS[neg.negotiationType] || neg.negotiationType}</Badge>
