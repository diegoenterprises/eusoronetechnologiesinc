...
export default function CatalystPackets() {
  const [, setLocation] = useLocation();
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");

  const packetsQuery = (trpc as any).catalystPackets.list.useQuery({ status: statusFilter === "all" ? undefined : statusFilter, limit: 50 });
  const summaryQuery = (trpc as any).catalystPackets.getSummary.useQuery();

  const sendMutation = (trpc as any).catalystPackets.send.useMutation({
  ...
  const filteredPackets = (packetsQuery.data as any)?.filter((packet: any) =>
    !searchTerm || packet.catalystName?.toLowerCase().includes(searchTerm.toLowerCase()) || packet.mcNumber?.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage catalyst onboarding packets</p>
        </div>
          ...
          {packetsQuery.isLoading ? (
            ...
            <div className="divide-y divide-slate-700/50">
              {filteredPackets?.map((packet: any) => (
                <div key={packet.id} className={cn("p-4 hover:bg-slate-700/20 transition-colors cursor-pointer", packet.status === "expired" && "bg-red-500/5 border-l-2 border-red-500")} onClick={() => setLocation(`/catalyst-packets/${packet.id}`)}>
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-white font-medium">{packet.catalystName}</p>
