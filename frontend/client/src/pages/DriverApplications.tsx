...
export default function DriverApplications() {
  ...
  const applicationsQuery = trpc.drivers.getApplications.useQuery({ search, status });
  const statsQuery = trpc.drivers.getApplicationStats.useQuery();

  const approveMutation = trpc.drivers.approveApplication.useMutation({
    onSuccess: () => { toast.success("Application approved"); applicationsQuery.refetch(); statsQuery.refetch(); },
    onError: (error) => toast.error("Failed", { description: error.message }),
  });

  const rejectMutation = trpc.drivers.rejectApplication.useMutation({
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-emerald-400 bg-clip-text text-transparent">Driver Applications</h1>
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search applications..." className="pl-9 bg-slate-800/50 border-slate-700/50 rounded-lg" />
        </div>
