        ...
        <Card className={cc}><CardHeader className="pb-3"><CardTitle className={cn("flex items-center gap-2",tc)}><Truck className="w-5 h-5 text-purple-500"/>Equipment & Operations</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <div><label className={lb}>Equipment Types</label>
              <div className="flex flex-wrap gap-2">{["dry_van","reefer","flatbed","step_deck","lowboy","double_drop","conestoga","liquid_tank","gas_tank","cryogenic","hazmat_van","food_grade_tank","water_tank","auto_carrier","livestock","log_trailer","grain_hopper","bulk_hopper","pneumatic","dump_trailer","intermodal","curtainside"].map(eq=>(<button key={eq} onClick={()=>setEqTypes(eqTypes.includes(eq)?eqTypes.filter(e=>e!==eq):[...eqTypes,eq])} className={cn("px-3 py-1.5 rounded-full text-xs font-medium transition-all",eqTypes.includes(eq)?"bg-gradient-to-r from-[#1473FF] to-[#BE01FF] text-white":isLight?"bg-slate-100 text-slate-500":"bg-slate-800 text-slate-400")}>{eq.replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase())}</button>))}</div>
            </div>
