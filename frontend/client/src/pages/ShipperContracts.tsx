    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage shipper agreements and contracts</p>
        </div>
      ...
      <div className="flex flex-wrap gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <Input value={searchTerm} onChange={(e: any) => setSearchTerm(e.target.value)} placeholder="Search contracts..." className="pl-9 bg-white/[0.02] border-white/[0.06] rounded-lg" />
        </div>
          ...
          {contractsQuery.isLoading ? (
            ...
            <div className="divide-y divide-white/[0.04]">
              {filteredContracts?.map((contract: any) => (
                <div key={contract.id} className={cn("p-4 hover:bg-white/[0.04] transition-colors cursor-pointer", contract.status === "expiring" && "bg-orange-500/5 border-l-2 border-orange-500")} onClick={() => setLocation(`/shipper-contracts/${contract.id}`)}>
