/**
 * AGREEMENTS LIBRARY — Unified agreement management for shipper, carrier, broker
...
export default function AgreementsLibrary() {
  ...
  const typeLabel = (t: string) => {
    switch (t) {
      case "carrier_shipper": return "Carrier-Shipper";
    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1100px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className={mt}>{isBroker ? "Manage shipper & carrier contracts" : isCarrier ? "View & sign shipper agreements" : "Manage carrier agreements & contracts"}</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by agreement #, carrier, company..." className={cn("pl-10", ic)} />
        </div>
      ...
      {agQuery.isLoading ? (
        ...
        <Card className={cc}>
          <CardContent className="py-16 text-center">
            ...
            <p className={cn("text-sm mb-6", mt)}>{isCarrier ? "Agreements will appear here when shippers send you contracts to sign." : "Create your first agreement to start managing carrier contracts."}</p>
            {!isCarrier && <Button className="bg-gradient-to-r from-[#1473FF] to-[#BE01FF] text-white rounded-xl font-bold" onClick={() => setLocation("/agreements/create")}><Plus className="w-4 h-4 mr-2" />Create Agreement</Button>}
          </CardContent>
        </Card>
      ) : (
                  ...
                  <div className="flex items-center gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 flex-wrap">
                        <span className="text-xs text-slate-400 flex items-center gap-1"><Building2 className="w-3 h-3" />{ag.partyBCompany || ag.partyBName || "Carrier TBD"}</span>
