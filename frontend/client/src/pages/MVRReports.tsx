...
export default function MVRReports() {
  ...
  const reportsQuery = trpc.compliance.getMVRReports.useQuery({ search });
  const statsQuery = trpc.compliance.getMVRStats.useQuery();

  const requestMutation = trpc.compliance.requestMVR.useMutation({
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="relative max-w-sm">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search MVRs..." className="pl-9 bg-slate-800/50 border-slate-700/50 rounded-lg" />
      </div>
