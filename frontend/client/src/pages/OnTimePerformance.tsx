  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className={cn("rounded-xl", (summary?.onTimeRate ?? 0) >= 95 ? "bg-gradient-to-r from-green-500/20 to-emerald-500/20 border-green-500/30" : "bg-white/[0.02] border-white/[0.06]")}>
        ...
        <Card className="bg-white/[0.02] border-white/[0.06] rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              <div className={cn("p-3 rounded-full", (Number(summary?.trend) || 0) > 0 ? "bg-green-500/20" : "bg-red-500/20")}>
                {(Number(summary?.trend) || 0) > 0 ? <TrendingUp className="w-6 h-6 text-green-400" /> : <TrendingDown className="w-6 h-6 text-red-400" />}
              </div>
              <div>
                {summaryQuery.isLoading ? <Skeleton className="h-8 w-12" /> : (
                  <p className={cn("text-2xl font-bold", (Number(summary?.trend) || 0) > 0 ? "text-green-400" : "text-red-400")}>
                    {(Number(summary?.trend) || 0) > 0 ? "+" : ""}{summary?.trend}%
                  </p>
                )}
          ...
          {summaryQuery.isLoading ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-slate-500">
                {(summary?.onTimeRate ?? 0) >= (summary?.targetRate ?? 95) ? "Target achieved!" : `${((summary?.targetRate ?? 95) - (summary?.onTimeRate ?? 0)).toFixed(1)}% below target`}
              </p>
            </div>
          )}
        </CardContent>
