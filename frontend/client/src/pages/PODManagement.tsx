...
import {
  FileCheck, Search, Upload, Download, Camera,
...
export default function PODManagement() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "received": return <Badge className="bg-green-500/20 text-green-400 border-0"><CheckCircle className="w-3 h-3 mr-1" />Received</Badge>;
      case "pending": return <Badge className="bg-yellow-500/20 text-yellow-400 border-0"><Clock className="w-3 h-3 mr-1" />Pending</Badge>;
      case "missing": return <Badge className="bg-red-500/20 text-red-400 border-0"><AlertTriangle className="w-3 h-3 mr-1" />Missing</Badge>;
      case "exception": return <Badge className="bg-orange-500/20 text-orange-400 border-0">Exception</Badge>;
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Proof of Delivery tracking and management</p>
        </div>
        <Button className="bg-gradient-to-r from-cyan-600 to-emerald-600 hover:from-cyan-700 hover:to-emerald-700 rounded-lg">
          <Upload className="w-4 h-4 mr-2" />Upload POD
        </Button>
      </div>
          ...
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Total PODs</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                {summaryQuery.isLoading ? <Skeleton className="h-8 w-12" /> : (
                  <p className="text-2xl font-bold text-green-400">{summary?.received || 0}</p>
                )}
        ...
        <Card className="bg-slate-800/50 border-slate-700/50 rounded-xl">
          <CardContent className="p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                {summaryQuery.isLoading ? <Skeleton className="h-8 w-12" /> : (
                  <p className="text-2xl font-bold text-red-400">{summary?.missing || 0}</p>
                )}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="text-white font-medium">Load {pod.loadNumber}</p>
                        {getStatusBadge(pod.status)}
                      </div>
                      <p className="text-sm text-slate-400">{pod.origin} → {pod.destination}</p>
                      <p className="text-sm text-slate-400">Consignee: {pod.consigneeName}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-white font-medium">{pod.deliveryDate}</p>
                      <p className="text-xs text-slate-500">{pod.deliveryTime}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center gap-4 text-xs text-slate-500">
                      <span>Driver: {pod.driverName}</span>
                      {pod.signedBy && <span>Signed by: {pod.signedBy}</span>}
                    </div>
