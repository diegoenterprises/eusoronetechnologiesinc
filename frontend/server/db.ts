...
async function runSchemaSync(db: ReturnType<typeof drizzle>) {
  ...
  try {
    ...
    // Performance indexes for FMCSA heavy queries — deferred to background (don't block startup)
    setTimeout(async () => {
      ...
      logger.info("[SchemaSync] FMCSA performance indexes created (background)");
    }, 30000); // Run 30s after startup
