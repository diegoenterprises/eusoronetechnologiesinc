...
export const fuelRouter = router({
  ...
  getNearbyStations: protectedProcedure.input(z.object({ lat: z.number().optional(), lng: z.number().optional(), fuelType: z.string().optional(), limit: z.number().optional() })).query(async () => [{ id: "s1", name: "Pilot Flying J", distance: 2.5, dieselPrice: 3.45 }]),
  getPrices: protectedProcedure.input(z.object({ location: z.object({ lat: z.number(), lng: z.number() }).optional(), radius: z.number().optional(), fuelType: z.string().optional() }).optional()).query(async () => ({ stations: [{ id: "s1", name: "Pilot Flying J", diesel: 3.45, unleaded: 3.25, premium: 3.65, distance: 2.5 }], regions: [{ name: "Texas", avgPrice: 3.45 }, { name: "Louisiana", avgPrice: 3.55 }] })),
