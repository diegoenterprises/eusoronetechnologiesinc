...
function getRoleContext(role: string): RoleCtx {
  const m: Record<string, RoleCtx> = {
    SHIPPER: { perspective: "carrier_availability", primaryMetric: "Available Trucks", secondaryMetric: "Avg Carrier Rate", description: "Where carriers are available for your loads", defaultLayers: ["carrier_capacity","rate_heat","compliance_risk","weather_risk"], gradient: ["rgba(0,200,83,0.4)","rgba(0,150,136,0.6)","rgba(3,169,244,0.7)","rgba(156,39,176,0.85)"], zoneActions: ["post_load","view_carriers","set_rate_alert"] },
    BROKER: { perspective: "spread_opportunity", primaryMetric: "Margin Opportunity", secondaryMetric: "Load:Truck Spread", description: "Best arbitrage & margin zones", defaultLayers: ["spread_opportunity","freight_demand","carrier_capacity","rate_heat"], gradient: ["rgba(76,175,80,0.4)","rgba(255,193,7,0.6)","rgba(255,152,0,0.7)","rgba(244,67,54,0.85)"], zoneActions: ["find_carriers","post_counter","calc_margin"] },
    ...
    ADMIN: { perspective: "platform_health", primaryMetric: "Active Users", secondaryMetric: "Load Volume", description: "Platform-wide operational intelligence", defaultLayers: ["freight_demand","carrier_capacity","compliance_risk"], gradient: ["rgba(0,0,0,0)","rgba(33,150,243,0.4)","rgba(0,200,83,0.6)","rgba(255,193,7,0.7)","rgba(244,67,54,0.85)"], zoneActions: ["view_users","manage_zones","generate_report"] },
    SUPER_ADMIN: { perspective: "executive_intelligence", primaryMetric: "All Metrics", secondaryMetric: "System Performance", description: "Complete platform oversight & business intelligence", defaultLayers: ["freight_demand","carrier_capacity","compliance_risk","factoring_risk","incident_history"], gradient: ["rgba(0,0,0,0)","rgba(33,150,243,0.3)","rgba(76,175,80,0.5)","rgba(255,193,7,0.6)","rgba(244,67,54,0.8)","rgba(183,28,28,1)"], zoneActions: ["all_admin_actions","configure_system","export_data"] },
  };
...
const HOT_ZONES = [
  ...
  { id: "hz-bak", name: "Bakken Formation", center: { lat: 48.15, lng: -103.63 }, radius: 80, state: "ND", loadCount: 156, truckCount: 52, loadToTruckRatio: 3.00, surgeMultiplier: 1.50, avgRate: 3.95, topEquipment: ["TANKER","HAZMAT"], reasons: ["Crude oil production peak","Limited carrier availability"], peakHours: "24/7", hazmatClasses: ["3","2.1"], oversizedFrequency: "HIGH" },
...
export const hotZonesRouter = router({
  ...
  // Driver/Carrier opportunities near a position
