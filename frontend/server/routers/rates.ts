...
export const ratesRouter = router({
  ...
  getMarketRates: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      } catch (e) { logger.error("[rates] Failed to fetch lane rates:", e); return empty; }
    }),
  ...
  getFuelSurcharge: protectedProcedure
    .query(async () => {
      ...
      try {
        ...
        logger.error("[rates] Failed to fetch EIA fuel surcharge data:", e);
      }
      return {
  ...
  getTrends: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      } catch (e) { logger.error("[rates] Failed to fetch rate trends:", e); return { period: input.period, currentAvg: 0, previousAvg: 0, changePercent: 0, trend: 'stable' as const, forecast: { nextWeek: 0, nextMonth: 0, confidence: 0 }, factors: [] }; }
    }),
