...
export const scadaRouter = router({
  ...
  getOverview: protectedProcedure.query(async () => ({ terminals: 5, totalThroughput: 2250000, activeRacks: 45, alerts: 3 })),
  getTerminals: protectedProcedure.query(async () => [{ id: "t1", name: "Houston Terminal", status: "operational", racks: 12 }]),
  getTanks: protectedProcedure.input(z.object({ terminalId: z.string().optional() })).query(async () => [{ id: "tank1", name: "Tank 1", product: "diesel", level: 75, capacity: 50000 }]),
  getTankLevels: protectedProcedure.query(async () => [{ tankId: "t1", level: 75, capacity: 50000, product: "diesel" }]),
  getAlarms: protectedProcedure.input(z.object({ severity: z.string().optional() })).query(async () => [{ id: "a1", type: "low_level", severity: "warning", message: "Tank 3 below 20%" }]),
  getActiveAlarms: protectedProcedure.query(async () => [{ id: "a1", type: "low_level", acknowledged: false }]),
  getAlarmHistory: protectedProcedure.input(z.object({ limit: z.number().optional() })).query(async () => [{ id: "a1", type: "low_level", resolvedAt: "2025-01-22 15:00" }]),
  ...
  getDailyThroughput: protectedProcedure.query(async () => ({ total: 450000, byProduct: { diesel: 200000, unleaded: 180000, premium: 70000 } })),
});
