...
export const agreementsRouter = router({
  ...
  generate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const validTypes = ["carrier_shipper","broker_carrier","broker_shipper","carrier_driver","escort_service","catalyst_dispatch","terminal_access","master_service","lane_commitment","fuel_surcharge","accessorial_schedule","nda","factoring","custom"];
      ...
      try {
        ...
        try {
          await db.execute(sql`CREATE TABLE IF NOT EXISTS agreements (
            ...
            templateId INT,
            agreementType ENUM('carrier_shipper','broker_carrier','broker_shipper','carrier_driver','escort_service','catalyst_dispatch','terminal_access','master_service','lane_commitment','fuel_surcharge','accessorial_schedule','nda','factoring','custom') NOT NULL,
...
function getDefaultClauses(type: string) {
  ...
  const typeSpecific: Record<string, any[]> = {
    ...
    catalyst_dispatch: [
      { id: "dispatch_authority", title: "Dispatch Authority & Scope", body: "Party A (Catalyst/Dispatcher) is authorized to act as a dispatch service provider for Party B (Carrier) in securing freight and managing load assignments. Party A shall use best efforts to identify, negotiate, and secure profitable loads matching Party B's equipment, lane preferences, and operational capacity. Party A does not assume carrier liability, broker authority, or motor carrier operating authority. Party A acts solely as an agent of Party B for dispatch coordination purposes.", isModified: false },
      ...
      { id: "carrier_obligations", title: "Carrier Obligations & Communication", body: "Party B shall: (a) maintain valid FMCSA operating authority, insurance, and safety ratings throughout the term; (b) promptly communicate equipment availability, preferred lanes, and scheduling constraints; (c) accept or decline dispatched loads within two (2) hours of notification; (d) provide real-time location updates via the EusoTrip platform during transit; (e) immediately report any service failures, delays, accidents, or cargo issues. Party B retains sole authority to accept or reject any load offer.", isModified: false },
      { id: "dispatch_exclusivity", title: "Exclusivity & Non-Solicitation", body: "During the term and for {{nonCircumventionMonths}} months following termination, Party B shall not directly contact or solicit business from shippers, brokers, or consignees introduced by Party A through the EusoTrip platform without Party A's written consent. This non-solicitation does not apply to relationships that existed prior to this Agreement. Party A shall not dispatch loads to competing carriers on the same lane without Party B's knowledge during exclusive lane commitments.", isModified: false },
