...
export const agreementsRouter = router({
  ...
  generate: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const validTypes = ["catalyst_shipper","broker_catalyst","broker_shipper","catalyst_driver","escort_service","dispatch_dispatch","terminal_access","throughput","storage_service","master_service","lane_commitment","fuel_surcharge","accessorial_schedule","nda","factoring","custom"];
...
function getDefaultClauses(type: string) {
  ...
  const typeSpecific: Record<string, any[]> = {
    ...
    throughput: [
      { id: "throughput_commitment", title: "Volume Commitment & Minimum Throughput", body: "Party B (Shipper/Catalyst) commits to a minimum throughput volume of {{volumeCommitmentTotal}} barrels (or equivalent units) per {{volumeCommitmentPeriod}} through Party A's terminal facility. If actual throughput falls below 80% of the committed volume in any measurement period, Party B shall pay a deficiency fee equal to the per-unit terminal fee multiplied by the shortfall volume (\"Take-or-Pay\"). If throughput exceeds 120% of commitment, the parties shall negotiate a volume discount in good faith. Throughput is measured at the terminal's custody transfer meter.", isModified: false },
      { id: "throughput_scheduling", title: "Scheduling, Nominations & Allocation", body: "Party B shall submit monthly volume nominations no later than the 20th of the preceding month via the EusoTrip Nomination Portal. Weekly scheduling adjustments must be submitted by Thursday at 17:00 CT for the following week. Party A shall confirm allocations within two (2) business days. During periods of capacity constraint, allocations shall be prorated based on committed volumes. Force majeure volumes are excluded from proration. Party A reserves the right to schedule maintenance windows with thirty (30) days' advance notice.", isModified: false },
      { id: "throughput_rates", title: "Throughput Fees & Rate Structure", body: "Party B shall pay the following throughput fees: (a) Base Throughput Fee: ${{baseRate}} per barrel (or per unit as specified); (b) Pipeline Receipt Fee: as per the pipeline tariff schedule; (c) Rack Loading Fee: included in base throughput fee; (d) Additive Injection: $0.005 per gallon per additive; (e) Product Heating: at cost plus 15% for products requiring temperature maintenance; (f) Line Fill / System Loss Allowance: 0.25% of throughput volume. Fees are invoiced monthly; payment due within {{paymentTermDays}} days of invoice date. Annual rate escalation: CPI-adjusted, capped at 3%.", isModified: false },
      { id: "throughput_quality", title: "Product Quality & Specifications", body: "All products delivered into Party A's facility must meet the specifications set forth in Exhibit B (Product Quality Standards) and applicable ASTM/API standards. Party A shall test incoming product per its standard receiving protocols. Off-spec product may be rejected at Party A's discretion. If off-spec product contaminates facility inventory, Party B shall bear all costs of remediation, downgrading, and disposal. Product quality verification may use the EusoTrip SPECTRA-MATCH system where available. Party A shall maintain Chain of Custody records for all products.", isModified: false },
      { id: "throughput_measurement", title: "Measurement, Metering & Loss Allowance", body: "All volumes shall be measured using calibrated meters meeting API MPMS (Manual of Petroleum Measurement Standards) requirements. Meters shall be tested and calibrated quarterly per API Chapter 4. Temperature and pressure corrections per API Chapter 11. Discrepancies exceeding 0.5% between Party A and Party B measurements shall trigger a joint investigation. System loss allowance: 0.25% of throughput volume per calendar month. Losses exceeding the allowance shall be the responsibility of the party whose negligence caused the excess loss.", isModified: false },
      { id: "throughput_interconnection", title: "Interconnection & Pipeline Access", body: "Party A shall provide interconnection facilities for the receipt and delivery of products via: (a) pipeline connections as specified in Exhibit C; (b) truck loading rack with assigned rack numbers; (c) rail siding (if applicable); (d) marine dock (if applicable). Party B shall bear the cost of any new interconnection facilities required solely for Party B's use. Interconnection design and construction shall comply with DOT/PHMSA pipeline safety regulations (49 CFR Parts 192/195) and API standards.", isModified: false },
      { id: "throughput_environmental", title: "Environmental & Regulatory Compliance", body: "Both parties shall comply with: (a) EPA regulations (40 CFR Parts 260-280) for petroleum storage and handling; (b) SPCC Plan requirements (40 CFR Part 112); (c) Clean Air Act emission standards; (d) RCRA waste management requirements; (e) State environmental regulations applicable to the facility location. Party B shall be responsible for environmental liabilities arising from Party B's products while in Party A's custody, except to the extent caused by Party A's negligence. Both parties shall maintain environmental impairment liability insurance.", isModified: false },
    ],
    storage_service: [
      { id: "storage_capacity", title: "Storage Capacity & Tank Assignment", body: "Party A (Terminal Operator) shall provide Party B (Shipper/Catalyst) with dedicated or shared storage capacity as follows: (a) Tank(s): as designated in Exhibit A (Tank Assignment Schedule); (b) Product Type: {{cargoType}} or as specified per tank; (c) Nominal Capacity: as specified in Exhibit A; (d) Working Capacity: approximately 95% of nominal capacity; (e) Heel Volume: Party A retains the right to maintain operational heel. For shared (common) storage, Party B's inventory is tracked via book balance. Party A shall not commingle incompatible products without written consent.", isModified: false },
      { id: "storage_fees", title: "Storage Fees & Service Charges", body: "Party B shall pay the following fees: (a) Base Storage Fee: ${{baseRate}} per barrel per month (or as specified); (b) In/Out Fee: per the throughput rate schedule; (c) Product Heating: at cost plus 15% for temperature-sensitive products; (d) Blending Services: $0.02-0.05 per gallon depending on complexity; (e) Additive Injection: $0.005 per gallon per additive; (f) Tank Cleaning (between grades): at cost, estimated $2,000-$10,000 depending on tank size and product; (g) Inventory Management: included in base storage fee. Minimum monthly storage charge applies regardless of utilization.", isModified: false },
      { id: "storage_blending", title: "Blending & Product Handling Services", body: "Party A shall provide blending services as requested by Party B, subject to operational capacity and scheduling: (a) Inline blending during loading operations; (b) Batch blending in designated blending tanks; (c) Additive injection per Party B's specifications; (d) Product transfer between tanks. Party B shall provide blending recipes and specifications in writing at least forty-eight (48) hours prior to the scheduled blending operation. Party A warrants that blending operations will be performed in accordance with the provided specifications but does not warrant the suitability of Party B's recipes. Quality verification via SPECTRA-MATCH where available.", isModified: false },
      { id: "storage_inventory", title: "Inventory Management & Reporting", body: "Party A shall maintain accurate inventory records for all products stored on behalf of Party B. Party A shall provide: (a) Daily inventory reports via the EusoTrip Terminal Dashboard, including opening balance, receipts, deliveries, gains/losses, and closing balance; (b) Monthly reconciliation statements within five (5) business days of month-end; (c) Real-time tank gauging data where automated systems are available. Inventory variances exceeding 0.5% of book balance in any month shall trigger a joint physical measurement. Party B may request independent inspection at Party B's expense with forty-eight (48) hours' notice.", isModified: false },
      { id: "storage_product_integrity", title: "Product Integrity & Contamination", body: "Party A shall exercise reasonable care to maintain the quality and integrity of Party B's stored products. Party A shall: (a) maintain tanks in good condition, free of leaks, with functioning vapor recovery where required; (b) prevent contamination from incompatible products; (c) maintain temperature within specified ranges for temperature-sensitive products; (d) implement corrosion monitoring and tank inspection programs per API 653. If contamination occurs due to Party A's negligence, Party A shall bear the cost of product downgrade, replacement, or disposal. If contamination results from Party B's off-spec delivery, Party B bears all costs.", isModified: false },
      { id: "storage_title_risk", title: "Title, Risk of Loss & Insurance", body: "Title to products stored in Party A's facility remains with Party B at all times. Risk of loss or damage transfers to Party A upon confirmed receipt at the facility interconnection point and transfers back to Party B upon loading into Party B's (or Party B's designee's) transport equipment. Party A shall maintain: (a) Terminal operator's legal liability insurance covering stored products; (b) Pollution liability insurance; (c) Property insurance on terminal facilities. Party B shall maintain cargo insurance and product liability insurance as specified in the Insurance Requirements clause.", isModified: false },
      { id: "storage_regulatory", title: "Regulatory Compliance & Permits", body: "Party A represents that the terminal facility holds all required permits and licenses for product storage, including: (a) EPA SPCC Plan; (b) State environmental operating permits; (c) Local fire marshal approval; (d) OSHA Process Safety Management compliance (29 CFR 1910.119) where applicable; (e) API 2610 terminal operations compliance. Party A shall notify Party B within five (5) business days of any regulatory enforcement action, permit modification, or compliance issue that may affect stored products or terminal operations.", isModified: false },
    ],
    lane_commitment: [
