...
export const fuelCardsRouter = router({
  ...
  getRecentTransactions: protectedProcedure.input(z.object({ limit: z.number().optional() })).query(async () => [
  ...
  toggleStatus: protectedProcedure.input(z.object({ cardId: z.string(), active: z.boolean() })).mutation(async ({ input }) => ({
    success: true,
    cardId: input.cardId,
    active: input.active,
  })),
