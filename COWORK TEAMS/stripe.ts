...
class StripeService {
   ...
   * This validates that the webhook payload was actually sent by Stripe
   */
  verifyWebhookSignature(payload: string, signature: string): boolean {
    if (!STRIPE_WEBHOOK_SECRET) {
      console.error("[Stripe] STRIPE_WEBHOOK_SECRET not configured - cannot verify webhook");
      return false;
    }

    if (!payload || !signature) {
      console.error("[Stripe] Invalid webhook payload or signature provided");
      return false;
    }
    ...
    try {
      ...
      if (!timestamp || !providedSignature) {
        console.error("[Stripe] Invalid signature format - missing timestamp or v1 value");
        return false;
      }
      ...
      if (!isValid) {
        console.error("[Stripe] Webhook signature verification failed - signature mismatch");
        return false;
      }

      // Additional security: verify timestamp is within tolerance (5 minutes)
      const now = Math.floor(Date.now() / 1000);
      const timestampNum = parseInt(timestamp, 10);
      const timeTolerance = 300; // 5 minutes in seconds

      if (isNaN(timestampNum)) {
        console.error("[Stripe] Invalid timestamp in webhook signature");
        return false;
      }

      if (Math.abs(now - timestampNum) > timeTolerance) {
        console.error("[Stripe] Webhook timestamp outside acceptable tolerance window");
        return false;
      }

      return true;
    } catch (error) {
      console.error("[Stripe] Error verifying webhook signature:", error instanceof Error ? error.message : String(error));
      return false;
    }
  }
}
